...
class WandbRerankerTask(RerankTask):
    ...
    def predict_and_eval(self, init_path=None):
        ...
        if not init_path or init_path == "none":
            ...
            self.reranker.trainer.load_best_model(self.reranker, Path(init_path), do_not_hash=True)

        dirname = init_path.split("/")[-1] if init_path else "noinitpath"
        ...
        folds_fn = {f"s{i}": savedir / f"s{i}" / "test" for i in range(1, 6)}
    ...
    def evaluate(self):
        ...
        if len(reranker_runs) != len(self.benchmark.folds):
            ...
            print("available runs: ", reranker_runs.keys())
            return {
                "fold_test_metrics": fold_test_metrics,
