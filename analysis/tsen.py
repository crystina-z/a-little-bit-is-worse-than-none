...
def get_capreolus_task(args):
    ...
    return WandbRerankerTask(config)
...
def get_data_generator(args, task, batch_size=1):
    ...
    n_generated_data, batch = 0, defaultdict(list)
    for data in tqdm(dev_dataset, total=size):
        if len(list(batch.values())[0]) == batch_size:
            yield batch
...
def get_bert_activation(inputs, reranker):
    inp, mask, seg = inputs["input"], inputs["mask"], inputs["pos_seg"]
    bert_main_layer = reranker.bert.bert
    ...
    return pooled_output[0]  # cls token
...
def main():
    ...
    with Timer(desc="Preparing Bert output"):
        X = np.array([
            get_bert_activation(inputs, task.reranker) for inputs in get_data_generator(args=args, task=task)])
    ...
    pickle.dump({"original": X, "transformed": Y}, open("tmp.pkl"))
    plt.scatter(Y[:0], Y[:1])
